"""
Executor Node: LangGraph node that runs executor agents on tasks.
"""
...
async def executor_node(state: AgentState) -> Dict[str, Any]:
    """
    LangGraph node that executes the current task in the plan.
    Iterates through tasks and processes each one.
    """
    llm = get_llm_service()
    ...
    # Notify streaming
    if callback:
        step_label = f"🔧 {task.tool_name}" if task.type == "tool" else f"🤖 {task.name}"
        await callback({
            "type": "task_update",
            "content": f"Executing step {current_index + 1}/{len(tasks)}: {step_label}",
        })
    ...
    if task_results:
        prev_context = "\n".join([
            f"Previous step result: {r.get('result', 'N/A')}"
    ...
    task_result = {
        ...
        "status": result.get("status", TaskStatus.COMPLETED).value if hasattr(result.get("status", ""), 'value') else str(result.get("status", "")),
    }
    task_results.append(task_result)

    # Update task in tasks list
    tasks[current_index]["status"] = "completed"
    tasks[current_index]["result"] = result.get("result", "")

    # Notify tool result
    if callback and current_task_data["type"] == "tool":
        await callback({
            "type": "tool_result",
            "content": f"Tool '{current_task_data.get('tool_name')}' result: {result.get('result', '')[:500]}",
        })
